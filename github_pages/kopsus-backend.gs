...
function handleRequest(e) {
  try {
    ...
    switch (action) {
      ...
      case 'changeMyPassword':   result = changeMyPassword(payload);   break;
...
// Self-service: user yang sedang login mengganti password miliknya sendiri.
// Berbeda dari setUserPassword (khusus admin, tidak perlu tahu password lama).
// Di sini identitas user diambil dari SESSION TOKEN, bukan dari input klien,
// supaya user tidak bisa mengganti password user lain.
function changeMyPassword(p) {
  if (!p.token) return { ok: false, error: 'Sesi tidak valid. Silakan login ulang.' };
  const cached = CacheService.getScriptCache().get('ks_session_' + p.token);
  if (!cached) return { ok: false, error: 'Sesi sudah berakhir. Silakan login ulang.' };

  let sessionUser;
  try {
    sessionUser = JSON.parse(cached);
  } catch (e) {
    return { ok: false, error: 'Data sesi tidak valid.' };
  }

  const currentPassword = (p.currentPassword || '').toString();
  const newPassword     = (p.newPassword || '').toString();
  if (!currentPassword || !newPassword) {
    return { ok: false, error: 'Password saat ini dan password baru wajib diisi.' };
  }
  ...
  const ss = getSS();
  const sh = ss.getSheetByName(SH_USERS);
  if (!sh || sh.getLastRow() < 2) return { ok: false, error: 'Sheet Users tidak ditemukan.' };
  ...
  for (let i = 0; i < data.length; i++) {
    const rowUser = (data[i][iUser] || '').toString().trim().toLowerCase();
    if (rowUser !== username) continue;

    const currentHash = (data[i][iHash] || '').toString().trim();
    if (currentHash !== hashPassword(currentPassword)) {
      return { ok: false, error: 'Password saat ini salah.' };
    }

    sh.getRange(i + 2, iHash + 1).setValue(hashPassword(newPassword));
    return { ok: true };
  }
